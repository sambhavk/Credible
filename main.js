...
function call() {
    let MsgData, MsgFrom, MsgTo, MsgTopic, Msgkeys;
    db.fetch('select MSG_ID from messages order by MSG_ID desc limit 1;', function (data) {
        count = data[0].msg_id;
    });
...
function PreScreening(MsgData, Msgkeys, MsgFrom, MsgTopic, count) {
    ...
    let sql = `select MSG_ID,FINAL_CREDIBILITY_SCORE from messages where KEYWORDS='${Msgkeys}';`;
...
function ManualScreening(MsgData, MsgFrom, MsgTopic, count) {
    ...
    if (MsgTopic != "Others" && MsgTopic != undefined) {
        let heroes = [{ heroid: "", heroWgt: 0, number: "" }, { heroid: "", heroWgt: 0, number: "" }, { heroid: "", heroWgt: 0, number: "" }];
        let sql = `select hi.HERO_ID, hi.EXPERT, h.WHATSAPP_NO,h.WGT from heroes_interests hi, heroes h where hi.topics='${MsgTopic}' and hi.HERO_ID=h.HERO_ID order by WGT desc limit 3;`;
        db.fetch(sql, function (data) {
